...
import com.bkw.library.JSONException;
import com.bkw.library.deserializer.JavaBeanDeserializer;
import com.bkw.library.deserializer.ObjectDeserializer;
...
public class SerializeConfig {
    ...
    ObjectSerializer getObjectWriter(Class<?> clazz) {
        ...
        if (Map.class.isAssignableFrom(clazz)) {
            throw new JSONException("MyFastJson 暂未实现Map序列化");
        } else if (List.class.isAssignableFrom(clazz)) {
            throw new JSONException("MyFastJson 暂未实现List序列化");
        } else if (clazz.isArray()) {
            throw new JSONException("MyFastJson 暂未实现数组序列化");
        }
        // 省略N多匹配（仅做示例参考！！！）
        else {
    ...
    public ObjectDeserializer getDeserializer(Type type) {
        ...
        if (derializer == null) {
            if (type instanceof Class) {
                ...
                // 参数化类型（List<OrderInfo>）
                throw new JSONException("MyFastJson 暂未实现参数化类型反序列化");
            } else if (type instanceof WildcardType) {
                // 通配符类型（Class<? extends/super UserInfo>）
                throw new JSONException("MyFastJson  暂未实现通配符类型反序列化");
            } else {
                throw new JSONException("MyFastJson  暂未该类型反序列化");
            }
            // 加入缓存集合
